...
test("session bootstrap secrets bind thread, execution lease, scoped token, and mutation grant", () => {
  ...
  const secret = sessionSecret("task-1", "multiagent", "summarize general", "caller-123", "thread-1", 4, "message-1", "scoped.token", "approved-repair", grant);
  ...
  assert.equal(Buffer.from(secret.data["authority-scope"], "base64").toString("utf8"), "approved-repair");
