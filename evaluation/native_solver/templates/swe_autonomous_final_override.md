...
9. Before writing completed status, check the final validation text for
   ...
   - If the final diff changes Go source, derive affected packages from
     ...
     `validation-package=...`, source-owner ledger, contract scout validation
     plan, or worker attempted validation command. Changed-package validation
     alone is insufficient when source evidence names additional contract
     packages.
