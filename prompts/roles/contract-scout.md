...
- For a capability advertised by prod-mcp as `access=read|materialize`, `mutation=false`, with no approval roles, the contract scout may call it directly through the supervisor using `multiagent ops read`; this does not require an ops agent or reviewer. Record the selected runbook, exact operation/version, bounded parameters, and receipt evidence.
- For any mutating or approval-gated capability, remain read-only: record the reviewed operation contract and route execution to the `--role ops` agent after review.
...
- Pagination, cursor traversal, chunking, identifier resolution, and traversal of related records needed for the explicit output are bounded execution mechanics, not unresolved user choices. When the direct-read gate passes, the requesting confined role owns these mechanics; use the orchestrator and ops role only for mutating or approval-gated operations unless the runbook or caller explicitly makes them outcome-level decisions.
