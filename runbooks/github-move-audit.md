...
2. Set the phase to `discover` and use `github.read-audit-pr@1.0.0` to read that pull request through the deployment GitHub App installation. Record its current 40-character lowercase `headSha`.
3. Set the phase to `request`, operation to `github.request-move-audit@1.0.0`, and parameters to exactly `repository`, `pullRequest`, `expectedHeadSha`, and `mode` (`light`, `core`, or `thorough`).
