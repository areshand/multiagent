...
The application-owned Audit Logger deployment contract is:

- deploy `docker/audit-log/Dockerfile` as a separate long-lived workload;
- use a dedicated ServiceAccount/workload identity and private Service;
- mount one dedicated writable volume at `/var/lib/audit-log`;
- mount an Ed25519 private key read-only and set
  `AUDIT_LOG_SIGNING_KEY_FILE` to its path;
- mount a client authorization file read-only and set
  `AUDIT_LOG_CLIENTS_FILE` to its path;
- set a stable `AUDIT_LOG_LOGGER_ID` and signing-key ID;
...
- configure at most one active Audit Logger replica for a ledger volume. A
