"""Durable implementation lifecycle state machine and command-line interface."""
...
def configured_state_dir():
    """Resolve state storage when a command runs, not when this module imports."""
    root = Path(os.environ.get("MULTIAGENT_ROOT", os.getcwd()))
    return Path(os.environ.get("MULTIAGENT_STATE_DIR", root / ".multiagent"))


def workflow_dir(workflow_id):
    validate_id("workflow ID", workflow_id)
    return configured_state_dir() / "workflows" / workflow_id / "lifecycle"
...
def validate_committed_decision(decision_id, plan_id):
    decision_dir = configured_state_dir() / "decisions" / decision_id
...
def main(argv=None):
    """Run the workflow command-line interface."""
