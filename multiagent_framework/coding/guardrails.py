...
def required_public_symbols(issue: str, metadata: dict[str, object] | None = None) -> list[str]:
    ...
    # External task metadata can contain answer-shaped verifier fields. Coding
    # agents must derive symbols from the user-visible issue and repository.
    _ = metadata
    symbols: set[str] = set()
    patterns = [
...
def implementation_scope_blockers(
    ...
    """Return source-derived blockers without evaluator answer leakage."""
    ...
    if not diff.strip():
        blockers.append("no source diff is present; coding-task completion requires a non-empty implementation patch")
        return blockers
